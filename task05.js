...
function filterAnagrams(word, anagrams) {
    var standart = word.split('').sort().join('');
    return anagrams.filter(function (item) { return item.split('').sort().join('') === standart; });
} // массивы
exports.default = filterAnagrams;
...
// const result = array.map(a => a.Naz_Extr_Id);
// console.log(result);
var responce = JSON.parse(content);
var key = "Naz_Extr_Id";
var sorted = content.sort(function (user1, user2) { return user1[key] > user2[key] ? 1 : -1; });
console.log(sorted);
